...
# Best-effort soft-kill on clean PowerShell exit. Window hard-close is handled
# by console attachment (see Start-ServerProcess: -NoNewWindow keeps Python in
# the same console group so Windows kills it when the window closes).
...
function Start-ServerProcess {
    ...
    # -NoNewWindow keeps Python attached to this console (no CREATE_NEW_CONSOLE).
    # Windows will send CTRL_CLOSE_EVENT to all console-attached processes when the
    # window is closed, killing Python automatically without any job-object tricks.
    return Start-Process -FilePath $pythonExe -ArgumentList '-m', 'src.ui' -WorkingDirectory $RootDir -PassThru -NoNewWindow -RedirectStandardOutput $stdoutLog -RedirectStandardError $stderrLog
}
